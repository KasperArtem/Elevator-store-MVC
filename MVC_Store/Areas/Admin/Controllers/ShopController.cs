...
namespace MVC_Store.Areas.Admin.Controllers
{
    public class ShopController : Controller
    {
        ...
        public ActionResult AddProduct(ProductVM model, HttpPostedFileBase file)
        {
            ...
            // Створюєм необхідні посилання дерикторій
            ...
            // перевіряєм наявність дерикторії (якщо немає, створюєм)
            ...
            {
                // отримуєм розширення файлу
                string ext = file.ContentType.ToLower();

                // перевіряєм розширення файл
                if (ext != "image/jpg" &&
                    ext != "image/jpeg" &&
                    ext != "image/pjpeg" &&
                    ext != "image/gif" &&
                ...
                // Об'являєм змінну з ім'ям зображення
                string imageName = file.FileName;

                // Зберігаєм ім'я зображення в модель DTO
                using (Db db = new Db())
                {
                ...
                // Визначаєм шлях до оригінального і зменшеного зображення
                var path = string.Format($"{pathString2}\\{imageName}");
                var path2 = string.Format($"{pathString3}\\{imageName}");

                // зберігаєм оригінальне зображення
                file.SaveAs(path);

                // створуєм і зберігаєм зменшену копію
